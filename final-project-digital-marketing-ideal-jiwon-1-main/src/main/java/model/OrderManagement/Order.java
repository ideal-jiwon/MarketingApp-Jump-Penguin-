...
public class Order {
    ...
    public OrderItem newOrderItem(Product p, int actualPrice, int q, MarketChannelAssignment mca) {
        OrderItem oi = new OrderItem(p, actualPrice, q, this, mca);
        orderitems.add(oi);
        return oi;
    }
    ...
    public double calculateTotalAdvertisingCost() {
        double totalCost = 0;
        for (OrderItem oi : orderitems) {
            totalCost += oi.marketChannelAssignment.calculateAdvertisingCost(oi.quantity);
        }
        return totalCost;
    }
}
